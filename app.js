...
function startBot() {
  getConfigValues((config, error) => {
    ...
    bot.sendMessage(adminGroupId, `<b>【Node.js】</b>\n服务叒双又启动了一次~\n\n#${currentUpdateCategory}`, {parse_mode: 'HTML'});
    // 监听消息事件
    bot.on("message", (msg) => {
      if (adminGroupId==msg.chat.id) {
          if(msg.video){
              queryDatabase(
              `INSERT INTO sese_video (video_id,category,user_id) VALUES (?,?,?)`,
              [msg.video.file_id,currentUpdateCategory,msg.from.id],
              (insertResult, insertError) => {
                  if (insertError) {
                      console.error("数据库插入失败:", insertError.message);
                      return;
                  }
                  bot.sendMessage(adminGroupId, `<b>【视频上传成功】</b>\n<code>${msg.video.file_id}</code>\n\n#${currentUpdateCategory}`, {parse_mode: 'HTML'});
              }
            );
            ...
            const queryVideoCategories = "SELECT category, COUNT(*) AS count FROM sese_video WHERE status = 'approved' GROUP BY category;";
                            ...
                            queryDatabase(queryVideoCategories, [], (categoryResults, categoryErr) => {
                                ...
                                if (categoryResults.length > 0) {
                                    categoryResults.forEach(cat => {
                                        categoryStats += `🎞️ <b>${cat.category}：</b> <code>${cat.count}</code> 部\n`;
                                    });
                                } else {
          ...
          queryDatabase(queryUsers, [], (users, err) => {
            ...
            bot.sendMessage(msg.chat.id, `上传的视频分类已设置为：${currentUpdateCategory}`);
        }
...
`;
                  ...
                  if (results.length === 0) {
                      ...
                      queryDatabase(
                          ...
                          (insertResult, insertError) => {
                              ...
                              bot.sendMessage(adminGroupId, `📣 <b>【${fullName}】</b> 注册成功！`, { 
                                parse_mode: "HTML",
                              });
                            if (msg.text.split(" ")[1]) {
            ...
            queryDatabase(queryUserStats, [telegramId, telegramId, telegramId], (userStats, err) => {
                ...
                queryDatabase(querySelectedCategory, [telegramId], (userCategory, err) => {
                    if (err) {
                        console.error("查询用户分类错误:", err);
                        return bot.sendMessage(telegramId, "❌ 获取用户分类失败，请稍后重试。");
                    }
                    ...
                    queryDatabase(queryCategories, [], (categories, err) => {
                        if (err) {
                            ...
                            return bot.sendMessage(telegramId, "❌ 获取视频分类失败，请稍后重试。");
                        }
                        ...
                        categoryButtons.unshift({
                            text: (current_category === "全部" ? '✅ ' : '') + "全部",
                            callback_data: "select_category:全部"
                        });
        
                        // 4 个一行的按钮排列
                        let inline_keyboard = [];
                        for (let i = 0; i < categoryButtons.length; i += 4) {
                            inline_keyboard.push(categoryButtons.slice(i, i + 4));
                        }
        
                        // 发送消息，新增投稿数量信息
                        bot.sendMessage(telegramId, `<b>我的数据：</b>\n📅 今日观看：<code>${today_views}</code> 部\n📺 累计观看：<code>${total_views}</code> 部\n📤 投稿数量：<code>${total_uploads}</code> 部\n\n请选择你想看的视频类型 👇`, {
        ...
        queryDatabase(`SELECT balance, current_category FROM sese_user WHERE user_id = ?`, [userId], (results, err) => {
            ...
            if (results.length > 0 && results[0].balance >= 1) {
                ...
                queryDatabase(updateQuery, [userId], (updateResults, updateErr) => {
                    ...
                    // 如果选择的是 "全部"，则不按分类筛选
                    const videoQuery = currentCategory === "全部"
                        ? `SELECT * FROM sese_video WHERE status = 'approved' ORDER BY RAND() LIMIT 1;`
                        : `SELECT * FROM sese_video WHERE category = ? AND status = 'approved' ORDER BY RAND() LIMIT 1;`;
    
                    const queryParams = currentCategory === "全部" ? [] : [currentCategory];
                    ...
                    queryDatabase(videoQuery, queryParams, (videoResults, videoErr) => {
                        ...
                        if (videoResults.length > 0) {
                            ...
                            bot.sendMessage(callbackQuery.message.chat.id, "❌ 没有找到可用的视频！", { parse_mode: "HTML" });
                        }
                    });
                });
            } else {
        ...
        queryDatabase(queryLastView, [chatId], (results, err) => {
            ...
            if (diffMinutes > 2) {
                ...
                queryDatabase(deductPointsQuery, [chatId], (updateResults, updateErr) => {
                    ...
                    // 继续换视频
                    sendRandomVideo(chatId, msg.message_id);
                });
            } else {
        ...
        queryDatabase(updateCategoryQuery, [selectedCategory, telegramId], (updateResult, updateErr) => {
            ...
            queryDatabase(queryUserStats, [telegramId, telegramId, telegramId], (userStats, statsErr) => {
                ...
                queryDatabase(queryCategories, [], (categories, catErr) => {
                    if (catErr) {
                        console.error("查询分类错误:", catErr);
                        return bot.sendMessage(telegramId, "❌ 获取视频分类失败，请稍后重试。");
                    }
    
                    let categoryButtons = categories.map(cat => ({
                        text: (selectedCategory === cat.category ? '✅ ' : '') + cat.category,
                        callback_data: `select_category:${cat.category}`
                    }));
    
                    // 添加“全部”分类
                    categoryButtons.unshift({
                        text: (selectedCategory === "全部" ? '✅ ' : '') + "全部",
                        callback_data: "select_category:全部"
                    });
...
function sendRandomVideo(chatId, messageId) {
    queryDatabase(`SELECT current_category FROM sese_user WHERE user_id = ?`, [chatId], (results, err) => {
        if (err || results.length === 0) {
            console.error("查询用户分类失败:", err);
            return bot.sendMessage(chatId, "❌ 获取分类失败，请稍后重试。");
        }
        ...
        queryDatabase(videoQuery, queryParams, (videoResults, videoErr) => {
            if (videoErr || videoResults.length === 0) {
                console.error("查询视频失败:", videoErr);
                return bot.sendMessage(chatId, "❌ 没有找到可用的视频！", { parse_mode: "HTML" });
            }
            ...
            queryDatabase(`SELECT * FROM sese_user WHERE user_id = ?;`, [video.user_id], (userResults, userErr) => {
                ...
                const uploader = userResults[0].user_name
